...
//審核通過保姆總數 - 數值
let url = "/SitterChartApi/GetApprovedSitter"
let quantity = document.querySelector("#approved-quantity")
httpGet(url).then((res) => {
    if (res.status == 20000) {
        quantity.innerText = res.data.ApprovedSitter
    } else {
...
//保姆狀態分析 - 圓餅圖
let statusQuantity = document.querySelector("#status-quantity")
let statusUrl = "/SitterChartApi/CountStatusQuantity"
httpGet(statusUrl).then((res) => {
    if (res.status == 20000) {
        ...
        CreatePieChart(statusQuantity, labels, datas)
    } else {
...
let createQuantity = document.querySelector("#create-quantity")
let createUrl = "/SitterChartApi/CountCreateQuantity"
httpGet(createUrl).then((res) => {
    if (res.status == 20000) {
        ...
        CreateLineChart(createQuantity, labels, "保姆申請數 / 月", resData)
    } else {
    ...
    httpGet(url)
        .then(res => {
            ...
            if (res.status == 20000) {
                ...
                console.log(`${topTitle}錯誤：${res.Message}`)
            }
        })
}